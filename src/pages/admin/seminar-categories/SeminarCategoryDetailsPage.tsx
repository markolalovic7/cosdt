...
function SeminarCategoryDetailsPage() {
  const [record, setRecord] = useState<SeminarCategory>();
  ...
  async function loadData() {
    try {
      ...
      let record = new SeminarCategory();
      if (id) {
        record = await api.seminarCategory.get(id);
      }
      setRecord(record);
  ...
  async function onFinish(rec: SeminarCategory) {
    try {
      setActionInProgress(true);
      const updatedRecord = { ...record, ...rec };
      let response;
      if (!id) {
        response = await api.seminarCategory.create(updatedRecord);
        SuccessNotification("Seminar category createda.");
      } else {
        response = await api.seminarCategory.update(updatedRecord);
        SuccessNotification("Seminar category changed.");
      }
      setRecord(response);
  ...
  return (
    <DataGridModalComponent
      title={id ? "Edit seminar category" : "New seminar category"}
...
export default SeminarCategoryDetailsPage;
