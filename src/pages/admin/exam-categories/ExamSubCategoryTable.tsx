...
interface ExamSubCategoryProps {
  parentId: number;
}

function ExamSubCategoryTable({ parentId }: ExamSubCategoryProps) {
  const [loading, setLoading] = useState<FetchStateEnum>(FetchStateEnum.NONE);
  const [records, setRecords] = useState<Array<ExamSubcategory>>([]);
  const [params, setParams] = useState<DataGridParamsType<ExamSubcategory>>(
    defaultPaginationSort()
  );
  const sorter = params.sorter as SorterResult<ExamSubcategory> | null;
  let { path, isExact } = useRouteMatch();
  let history = useHistory();

  const columns: Array<DataGridColumnType<ExamSubcategory>> = [
    {
      title: "Name",
      dataIndex: "name",
      editable: true,
      ...
      link: {
        url: (record) =>
          `${path}/${parentId}${AdminRoutesEnum.EXAM_SUB_CATS}/${record.id}`,
      },
      rules: [
        {
          required: true,
  ...
  async function loadData() {
    try {
      ...
      let records = await api.examSubcategory.getAll();
      setRecords(records);
  ...
  async function handleCreate() {
    history.replace(
      `${path}/${parentId}${AdminRoutesEnum.EXAM_SUB_CATS}/new`
    );
  }

  async function handleUpdate(record: ExamSubcategory): Promise<void> {
    try {
      setLoading(FetchStateEnum.LOADING);
      let data = [...records];
      const recordIndex = data.findIndex((rec) => rec.id === record.id);
      const updatedRecord: ExamSubcategory = {
        ...data[recordIndex],
        ...record,
      };
      let response = await api.examSubcategory.update(updatedRecord);
  ...
  async function handleDelete(id: number) {
    try {
      setLoading(FetchStateEnum.LOADING);
      await api.examSubcategory.delete(id);
      let data = [...records];
  ...
  function handleChange(
    ...
    sorter:
      | SorterResult<ExamSubcategory>
      | SorterResult<ExamSubcategory>[]
  ) {
    setParams(
      updateDataGridParams<ExamSubcategory>(
        pagination,
        filters,
        sorter,
        columns
  ...
  return (
    <DataGridComponent<ExamSubcategory>
      bordered
      columns={columns}
      rowKey={(rec) => rec.id}
      dataSource={records}
...
export default React.memo(ExamSubCategoryTable);
