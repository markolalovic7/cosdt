...
function LocationDetailsPage() {
  const [record, setRecord] = useState<Location>();
  ...
  async function loadData() {
    try {
      ...
      if (id) {
        record = await api.location.get(id);
      }
      setRecord(record);
  ...
  async function onFinish(rec: Location) {
    try {
      setActionInProgress(true);
      const updatedRecord = { ...record, ...rec };
      let response;
      if (!id) {
        response = await api.location.create(updatedRecord);
        SuccessNotification("Organizator created.");
      } else {
        response = await api.location.update(updatedRecord);
  ...
  return (
    <DataGridModalComponent
      title={id ? "Edit Location" : "New Location"}
