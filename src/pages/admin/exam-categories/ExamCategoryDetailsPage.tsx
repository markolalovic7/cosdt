...
function ExamCategoryDetailsPage() {
  const [record, setRecord] = useState<ExamCategory>();
  ...
  async function loadData() {
    try {
      ...
      let record = new ExamCategory();
      if (id) {
        record = await api.examCategory.get(id);
      }
      setRecord(record);
  ...
  async function onFinish(rec: ExamCategory) {
    try {
      setActionInProgress(true);
      const updatedRecord = { ...record, ...rec };
      let response;
      if (!id) {
        response = await api.examCategory.create(updatedRecord);
        SuccessNotification("Exam category created.");
      } else {
        response = await api.examCategory.update(updatedRecord);
  ...
  return (
    <DataGridModalComponent
      title={id ? "Edit exam category" : "New exam category"}
...
export default ExamCategoryDetailsPage;
