...
function SurveyBuilderPageDetails() {
  ...
  async function loadData() {
    try {
      ...
      if (id) {
        record = await api.seminarSurvey.get(id);
        // @ts-ignore
  ...
  async function onFinish(rec: FormSurvey) {
    try {
      ...
      if (!id) {
        await api.seminarSurvey.create(updatedRecord);
        SuccessNotification("Form created.");
      } else {
        await api.seminarSurvey.update(response);
  ...
  return (
    ...
    >
      <h1>{id ? "Edit survey form" : "Create survey form"}</h1>
      <div>
...
export default SurveyBuilderPageDetails;
