...
function LocationsPage() {
  const [records, setRecords] = useState<Array<Location>>([]);
  const [loading, setLoading] = useState<FetchStateEnum>(FetchStateEnum.NONE);
  const [params, setParams] = useState<DataGridParamsType<Location>>(
    defaultPaginationSort()
  );
  let { path, isExact } = useRouteMatch();
  let history = useHistory();
  const sorter = params.sorter as SorterResult<Location> | null;

  const columns: Array<DataGridColumnType<Location>> = [
    {
      title: "Name",
      dataIndex: "name",
      editable: true,
  ...
  async function loadData() {
    try {
      ...
      let records = await api.location.getAll();
      setRecords(records);
  ...
  async function handleUpdate(record: Location): Promise<void> {
    try {
      setLoading(FetchStateEnum.LOADING);
      let data = [...records];
      let response = await api.location.update(record);
  ...
  function handleChange(
    ...
    sorter: SorterResult<Location> | SorterResult<Location>[]
  ) {
    setParams(
      updateDataGridParams<Location>(pagination, filters, sorter, columns)
    );
  }
